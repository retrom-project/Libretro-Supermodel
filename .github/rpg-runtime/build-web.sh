...
work=$(mktemp -d "$root/.retrom-build/retrom-supermodel-web.XXXXXX")
...
if ! docker run --rm --platform linux/amd64 --hostname retrom-supermodel \
  ...
  /recipe/build-emulatorjs-core.sh \
...
stage="$work/stage"
mkdir -p "$stage"
install -m 0644 "$work/raw/supermodel_libretro.js" "$stage/"
install -m 0644 "$work/raw/supermodel_libretro.wasm" "$stage/"
cat "$root/Docs/LICENSE.txt" "$work/raw/retroarch-COPYING" > "$stage/license.txt"
printf '%s\n' '{"minimumEJSVersion":"4.3.0-pre","version":"0.3a-WIP"}' > "$stage/build.json"
printf '%s\n' '{"name":"supermodel","extensions":["zip"],"makeoptions":{"buildpath":"./","makescript":"Makefile","arguments":["platform=emscripten"]},"options":{},"save":true,"license":"Docs/LICENSE.txt","repo":"https://github.com/retrom-project/Libretro-Supermodel"}' > "$stage/core.json"
chmod 0644 "$stage/build.json" "$stage/core.json" "$stage/license.txt"

(cd "$stage" && 7z a -mtm=off -mta=off -mtc=off -bd -bso0 -bsp0 -t7z "$output/supermodel-wasm.data" \
  supermodel_libretro.js supermodel_libretro.wasm build.json core.json license.txt)
